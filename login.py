...
from train import Train
# from FaceAttendance import face_recognition
...
class Login:
    def __init__(self,root):
        ...
        self.bg=ImageTk.PhotoImage(file=r"college_images\iStock-182059956_18390_t12.jpg")
        
        lb1_bg=Label(self.root,image=self.bg)
        lb1_bg.place(x=0,y=0, relwidth=10,relheight=10)

        frame1= Frame(self.root,bg="#002B53")
        frame1.place(x=520,y=170,width=400,height=550)
        ...
        lb1img1.place(x=650,y=175, width=100,height=100)
    ...
    def login(self):
        if (self.txtuser.get()=="" or self.txtpwd.get()==""):
            ...
            if row==None:
                ...
                open_min=messagebox.askyesno("YesNo","Access only granted to Teachers")
                if open_min>0:
                    self.new_window=Toplevel(self.root)
                    # self.app=Face_(self.new_window)
                    #root.withdraw()
                    #root.destroy()
                else:
                    if not open_min:
